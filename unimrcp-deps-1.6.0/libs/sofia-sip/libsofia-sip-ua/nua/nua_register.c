				...
				tagi_t const *tags)
{
  ...
  if (nr) {
    ...
    if (nr->nr_by_stack) {
      sip_contact_t *m_l = nr->nr_contact, *previous = NULL;

      outbound_get_contacts(nr->nr_ob, &m_l, &previous);

      sip_add_dup(msg, sip, (sip_header_t *)m_l);
...
nua_stack_init_registrations(nua_t *nua)
{
  ...
  if (v) {
    ...
    sip_via_init(v_l)->v_next = v + 1;
    v_l[0].v_protocol = sip_transport_udp;
    v_l[0].v_host = "addr.is.invalid.";
    sip_via_init(v_l + 1);
    v_l[1].v_protocol = sip_transport_tcp;
    v_l[1].v_host = "addr.is.invalid.";

    nua_registration_from_via(nr_list, dnh, v_l, 0);
  }
					 ...
					 ...)
{
  ...
  if (transport) {
    ...
    if (strlen(transport) < (sizeof _transport)) {
      char *s_l = strcpy(_transport, transport);
      short c;
