						...
						)
{
	...
	*/
	LOG_INFO(" Recog Channel Init Success ID [ %d ]", m_id);
	return TRUE;
}
...
void CAliChannel::RecogMain()
{
	...
	while (m_starting)
	{
		...
		if (APR_EOF == stu) {
			...
			//LOG_NOTICE("APR_EINTR  Frame Queue exit Msg:%s", CAliUtil::aprErrorToStr(stu).c_str());
			break;
		}
		...
		if (nullptr != frame) {
			...
			if (m_Recoger->DoWriteFrame(frame) == -1) {
				//LOG_NOTICE("<------ Send Audio Data Failed -------->");
				break;
			}
			RecordMain(frame);
		}
