...
bool	CAliRecogEngine::EngineInit(mrcp_engine_t* engine)
{
	...
	m_RecogMod = brecoger ? RECOGNIZER : TRANSCRIBER;

	LOG_INFO("<---------- Ali Recog Engine Init For [ %s ] :%s ----------->", recoger, brecoger ? "true" : "false");
...
string	CAliRecogEngine::EngineRecogCompleted(EngineChannel * pCh)
{
	...
	return pCh->AliRe->DoRecogResultGet();
}


const recog_mod&	CAliRecogEngine::EngineRecogModule()const
{
	return m_RecogMod;
}
