...
MRCP_PLUGIN_DECLARE(mrcp_engine_t*) mrcp_plugin_create(apr_pool_t *pool)
{
	...
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine plugin createe ===========>");
	/* create engine base */
...
static apt_bool_t Ali_synth_engine_destroy(mrcp_engine_t *engine)
{
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine plugin destroy ===========>");
...
static apt_bool_t Ali_synth_engine_open(mrcp_engine_t *engine)
{
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine plugin open ===========>");
	Ali_synth_engine_t *Ali_engine = (Ali_synth_engine_t *)engine->obj;
	if(Ali_engine->task) {
		apt_task_t *task = apt_consumer_task_base_get(Ali_engine->task);
		apt_task_start(task);
	...
	if (!SynthEngine::synthgEngine->EngineInit(engine)) {
		LOG_ERROR("<==== Ali Synth Engine init failed ====>");
		SynthEngine::synthgEngine = nullptr;
	}
...
static apt_bool_t Ali_synth_engine_close(mrcp_engine_t *engine)
{
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine plugin close ===========>");
	Ali_synth_engine_t *Ali_engine = (Ali_synth_engine_t *)engine->obj;
	if(Ali_engine->task) {
		apt_task_t *task = apt_consumer_task_base_get(Ali_engine->task);
...
static mrcp_engine_channel_t* Ali_synth_engine_channel_create(mrcp_engine_t *engine, apr_pool_t *pool)
{
	...
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine channel create ===========>");
...
{
	/* nothing to destroy */
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine channel destroy ===========>");
	return TRUE;
}
...
static apt_bool_t Ali_synth_channel_open(mrcp_engine_channel_t *channel)
{
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine channel open ===========>");
...
static apt_bool_t Ali_synth_channel_close(mrcp_engine_channel_t *channel)
{
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine channel close ===========>");
...
static apt_bool_t Ali_synth_channel_request_process(mrcp_engine_channel_t *channel, mrcp_message_t *request)
{
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine channel request process ===========>");
...
static apt_bool_t Ali_synth_channel_speak(mrcp_engine_channel_t *channel, mrcp_message_t *request, mrcp_message_t *response)
{
	...
	//synth_channel->time_to_complete = 0;
	//if(channel->engine) {
	//	char *file_name = apr_psprintf(channel->pool,"Ali-%dkHz.pcm",descriptor->sampling_rate/1000);
	...
	//		apt_log(ALI_SYNTH_LOG_MARK,APT_PRIO_INFO,"Set [%s] as Speech Source " APT_SIDRES_FMT,
	//			file_path,
	//			MRCP_MESSAGE_SIDRES(request));
	//	}
	//	else {
	//		apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= No Speech Source :%s ===========>", file_path);
	//		apt_log(ALI_SYNTH_LOG_MARK,APT_PRIO_INFO,"No Speech Source [%s] Found " APT_SIDRES_FMT,
	//			file_path,
	//			MRCP_MESSAGE_SIDRES(request));
	...
	if (SynthEngine::synthgEngine) {
		...
		if (!SynthEngine::synthgEngine->EngineSynthStart(synth_channel, speakTxt.c_str())) {
			LOG_ERROR("<==== Start Ali Synth failed speakTxt :%s ====>", speakTxt.c_str());
		}
	}
...
static apt_bool_t Ali_synth_channel_stop(mrcp_engine_channel_t *channel, mrcp_message_t *request, mrcp_message_t *response)
{
	...
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine channel stop ===========>");
	return TRUE;
}
...
static apt_bool_t Ali_synth_stream_destroy(mpf_audio_stream_t *stream)
{
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine stream destroy ===========>");
	return TRUE;
}
...
static apt_bool_t Ali_synth_stream_open(mpf_audio_stream_t *stream, mpf_codec_t *codec)
{
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine stream open ===========>");
	return TRUE;
}
...
static apt_bool_t Ali_synth_stream_close(mpf_audio_stream_t *stream)
{
	apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine stream close ===========>");
...
static apt_bool_t Ali_synth_stream_read(mpf_audio_stream_t *stream, mpf_frame_t *frame)
{
	...
	//apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine stream read ===========>");
	Ali_synth_channel_t *synth_channel = (Ali_synth_channel_t *)stream->obj;
	...
	//	if(completed) {
	//		apt_log(ALI_SYNTH_LOG_MARK, APT_PRIO_INFO, "<========= synth engine stream read completed ===========>");
